...
# How to use the graph ? Example: compute a shortest path...
...
# get data
location_name = "Lyon"
poi_from_location_gdf = OsmGt.poi_from_location("Lyon").get_gdf()
roads_from_location = OsmGt.roads_from_location("Lyon",	mode="pedestrian", poi_from_location_gdf)
...
start_node_wkt = poi_from_web_found_gdf[poi_from_web_found_gdf['topo_uuid'] == start_node_topo_uuid].iloc[0]["geometry"].wkt
end_node_wkt = poi_from_web_found_gdf[poi_from_web_found_gdf['topo_uuid'] == end_node_topo_uuid].iloc[0]["geometry"].wkt
...
shortest_path_found = network_data[network_data['topo_uuid'].isin(roads_ids)]
```
